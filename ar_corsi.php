...
<body class="">
  <div class="wrapper ">
    ...
    <div class="main-panel">
      ...
      <div class="content">
        <div class="listacorsi">
          <h4>Corsi a cui sei iscritto:</h4>
          <?php
          //devo visualizzare le iscrizioni e le prenotazioni agli eventi
          //con possibilità di modifica
          $DB = databaseConnection($servername, $username, $dbpassword, $dbname);
          $record = readTableRel($_SESSION['datistudente']['id'], 'corsistudenti', $DB);
          $outcorsi = "";
          //var_dump($record);
          if (count($record) < 1) {
            $outcorsi = "<p>Non ancora sei iscritto a nessun corso,
              ti puoi iscrivere ai corsi <a href='corsi.php'>qui</a></p>";
          } else {

            for ($i = 0; $i < count($record) - 1; $i++) {
              $corsi = readRecordtable($record[$i]['idcorso'], 'corsi', $DB);

              $outcorsi .= '<h6>' . $corsi['nome'] . '</h6>
              <div class="row el_listacorsi">
                <div class="col-3 overflow-hidden imglist" style="background-image: url(&quot;' . $corsi['img'] . '&quot;)">
                        
                </div>
                    <div class="col text-justify">
                        <p>' . $corsi['descrizione'] . '</p>
                        <a href="corso.php?id=' . $corsi['id'] . '" target="_blank">Vai al corso <i class="bi bi-arrow-right"></i></a>
                    </div>
                    <div style="display: block">
                      <button class="btn btn-danger"><a href="#" style="color:white">Cancella iscrizione</a></button>
                    </div>
